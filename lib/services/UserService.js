...
const forgotPassword = async (email, res) => {
  ...
  const user = await User.findUserByEmail(email);
  if (!user) next({ message: 'Could not find email' });
  ...
  // send for success state
  res
    .status(200)
    .send({ message: 'Please check your email for a reset link!' });
};

const setNewPassword = async (tempToken, newPassword, email, res) => {
  // check token
  jwt.verify(tempToken, process.env.RESET_SECRET, (error) => {
    if (error) res
      .status(401)
      .send({ message: 'Sorry, please try again. Token may have expired or is incorrect' });
  });
  ...
  // if no user
  if (!user) next({ message: 'Sorry, we could not find a match for this link' });
  ...
  // TODO: cannot log after done error
  res
    .status(200)
    .send({ message: 'Password was updated successfully' });
};
