...
const authorize = async ({ email, password }) => {
  ...
  // if there's no user
  if (!user) throw new Error('Sorry, incorrect email/password');

  // verify users password
  const validPassword = await comparePasswords(password, user.passwordHash);

  // invalid pw scenario
  if (!validPassword) throw new Error('Sorry, incorrect email/password');
...
const forgotPassword = async (email) => {
  ...
  const tempToken = await setTemporaryToken(user);
